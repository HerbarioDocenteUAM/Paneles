create_panel <- function(data = data,
                                   path = NULL,
                                   filename = NULL,
                                   title = NULL,
                                   subtitle = NULL,
                                   ...
                                   sinonimo.column = NULL,
                                   descripcion.column = NULL,
                                   edafico.column = NULL,
                                   piso.column = NULL,
                                   biotipo.column = NULL,
                                   raunkier.column = NULL,
                                   altitud.column = NULL,
                                   usos.column = NULL,
                                   reproduccion.column = NULL,
                                   hoja.column = NULL,
                                   filotaxis.column = NULL,
                                   localidad.column = NULL,
                                   habito.column = NULL,
                                   lpic = NULL,
                                   rpic = NULL,
                                   fecha.column = NULL,
  ...
  sinonimo.column         <- check_column_or_create_empty_char(data, sinonimo.column)
  descripcion.column      <- check_column_or_create_empty_char(data, descripcion.column)
  edafico.column          <- check_column_or_create_empty_char(data, edafico.column)
  piso.column             <- check_column_or_create_empty_char(data, piso.column)
  biotipo.column          <- check_column_or_create_empty_char(data, biotipo.column)
  raunkier.column         <- check_column_or_create_empty_char(data, raunkier.column)
  descripcion.column      <- check_column_or_create_empty_char(data, descripcion.column)
  altitud.column          <- check_column_or_create_empty_char(data, altitud.column)
  usos.column             <- check_column_or_create_empty_char(data, usos.column)
  reproduccion.column     <- check_column_or_create_empty_char(data, reproduccion.column)
  hoja.column             <- check_column_or_create_empty_char(data, hoja.column)
  filotaxis.column        <- check_column_or_create_empty_char(data, filotaxis.column)
  ...
  rmarkdown::render(
    ...
    params = list(
      ...
      author.i           = if (author.column           == "") {bl.char} else {data[i,author.column]},
      vernaculo.i        = if (vernaculo.column        == "") {bl.char} else {data[i,vernaculo.column]},
      sinonimo.i         = if (sinonimo.column         == "") {bl.char} else {data[i,sinonimo.column]},
      descripcion.i      = if (descripcion.column      == "") {bl.char} else {data[i,descripcion.column]},
      edafico.i          = if (edafico.column          == "") {bl.char} else {data[i,edafico.column]},
      piso.i             = if (piso.column             == "") {bl.char} else {data[i,piso.column]},
      biotipo.i          = if (biotipo.column          == "") {bl.char} else {data[i,biotipo.column]},
      raunkier.i         = if (raunkier.column         == "") {bl.char} else {data[i,raunkier.column]},
      altitud.i          = if (altitud.column          == "") {bl.char} else {data[i,altitud.column]},
      usos.i             = if (usos.column             == "") {bl.char} else {data[i,usos.column]},
      reproduccion.i     = if (reproduccion.column     == "") {bl.char} else {data[i,reproduccion.column]},
      hoja.i             = if (hoja.column             == "") {bl.char} else {data[i,hoja.column]},
      filotaxis.i        = if (filotaxis.column        == "") {bl.char} else {data[i,filotaxis.column]},
      localidad.i        = if (localidad.column        == "") {bl.char} else {data[i,localidad.column]},
